...
\paragraph{Covariance coefficient}
	...
	$\\abs{\delta[i,j]}=1 \implies i=\alpha j$
...
$\forall$ stationary processes: $\abs{\rho(\tau)}\leq1	\quad\forall\tau$
...
\end{itemize}
$\abs{\gamma(\tau)}\leq\gamma(0)		\quad\forall\tau$
...
\begin{alignat*}{2}
	\Gamma_{yy}(\omega)&=\abs{W(e^{j\omega})}^2 & \cdot \Gamma_{uu}(\omega)\\
...
&=(c_0+c_1z^{-1}+\dots+c_nz^{-n})\eta(t)
\end{align*}

\paragraph{Transfer function}
\[W(z)
=c_0+c_1z^{-1}+\dots+c_nz^{-n}
=\frac{c_0z^n+c_1z^{n-1}+\dots+c_n}{z^n}
\]\\
All poles are in the complex origin

\paragraph{Mean}
\begin{alignat*}{2}
E[v(t)]&=(c_0+c_1+\dots+c_n)\underbrace{E[\eta(t)]}_0\\
\Aboxed{E[v(t)&=0}
\end{alignat*}
...
\gamma(\tau)=
\begin{cases}
\lambda^2\cdot \sum_{i=0}^{n-\tau} c_ic_{i-\tau}	&	\abs{\tau}\leq n\\
0	& \text{otherwise}
\end{cases}
}
\]
...
\begin{align*}
\gamma(0)&=(c_0^2+c_1^2+\dots+c_n^2)\lambda^2\\
\gamma(1)&=(c_0c_1+c_1c_2+\dots+c_{n-1}c_n)\lambda^2\\
\gamma(2)&=(c_0c_2+c_1c_3+\dots+c_{n-2}c_n)\lambda^2\\
&\dots\\
...
\paragraph{Model}
\[
v(t)=c_0\eta(t)+c_1\eta(t-1)+\dots+c_k\eta(t-k)+\dots=\sum_{i=0}^{\infty}c_i\eta(t-i)
\]
\paragraph{Variance}
\[
\gamma(0)
=(c_0^2+c_1^2+\dots+c_k^2+\dots) \lambda^2
=\lambda^2 \sum_{i=0}^{\infty} c_i^2
\]
\subsection{Well definition of an MA($\infty$)}
We need to have $\abs{\gamma(\tau)}\leq \gamma(0)$, so we must require that \\
...
To check if the variance is finite we check $\gamma(0)=\lambda^2 \sum_{i=0}^{\infty} a^{2i}<\infty$. The given is a geometric series, convergent for $\abs{a}<1$. Under this hypothesis its value is
\[
...
\hspace*{-1cm}\vbox{	%slightly move these equations to the right
 ...
 \begin{alignat*}{7}
\gamma(1)
	&=(c_0c_1+c_1c_2+\dots)\lambda^2
	&=(a+aa^2+\dots)\lambda^2
	&=a(1+a^2+a^4+\dots)\lambda^2
	&=a\lambda^2\sum_{i=0}^{\infty} a^{2i}
	&=a\frac{\lambda^2}{1-a^2}
	&=a\gamma(0)\\
\gamma(2)
	&=(c_0c_2+c_1c_3+\dots)\lambda^2
	&=(a^2+aa^3+...)\lambda^2
	&=a^2(1+a^2+a^4+\dots)\lambda^2
	&=a^2\lambda^2\sum_{i=0}^{\infty} a^{2i}
	&=a^2\frac{\lambda^2}{1-a^2}
	&=a^2\gamma(0)\\
\Aboxed{
\gamma(\tau)&=a^{\abs\tau}\frac{\lambda^2}{1-a^2}
}
\end{alignat*}
} 
...
\paragraph{Transfer function}
\[
W(z)=\frac{z^n}{z^n-a_1z_{n-1}-\dots-a_n}
\]
\paragraph{Mean}
\begin{alignat*}{6}
E[v(t)]	&=	a_1E[v(t-1])	&+&a_2E[v(t-2)]	&+&\dots	&+&a_nE[v(t-n)]	&+&\underbrace{E[\eta(t)]}_0\\
m		&=	a_1m			&+&a_2m			&+&\dots	&+&a_nm\\
...
\[
v(t)=
	a_1v(t-1)+\dots+a_{n_a}v(t-n_a)+
	c_0\eta(t)+\dots+c_{n_c}v(t-n_c)
\]
...
We want to predict $v(t+r)$ from $v(t), v(t-1), \dots$, where $r$ is called prediction horizon, of the following stationary process:
...
\[
v(t+r)=W(z)\eta(t+r)
= \underbrace{w_0 \eta(t+r)+w_1 \eta(t+r-1)+\dots+w_{r-1} \eta(t+1)}_{\alpha(t)\textbf{ unpredictable: }\text{future of }\eta\text{ involved}}
+ \underbrace{w_r \eta(t)+w_{r+1} \eta(t-1)+\dots}_{\beta(t) \textbf{ predictable}}
\]
The optimal fake predictor is then
\[
\boxed{
v(t+r|t)=w_r \eta(t)+w_{r+1}\eta(t-1)+\dots
}=\beta(t)
\]
...
\[
\boxed{
\epsilon(t)=w_0 \eta(t+r)+w_1 \eta(t+r-1)+\dots+w_{r-1} \eta(t+1)
}
\]
\[
\boxed{
Var[\epsilon(t)]=(w_0^2+w_1^2+\dots+w_{r-1}^2)\lambda^2
}
\]
...
\item \textbf{Family selection:} choose a family of models $M(\theta)$
	\begin{description}
	\item[MA(1)] $\theta=[a]$
	\item[MA(n)] $\theta=[a_1,\dots,a_n]$
	\item[ARMA($n_a,n_c$)] $\theta=[a_1,\dots,a_{n_a},
	c_1,\dots,c_{n_c}]$
	\item[\dots]
	\end{description}
\item \textbf{Select an optimization criterion}
	...
	\item[Mean absolute error] $J(\theta)=\frac{1}{N}\sum_{t=1}^N \abs{\epsilon_\theta(t)}$
	\item[\dots]
	\end{description}
...
\subsection{Least Square Error}
Consider the ARX model:
\[
y(t)=
a_1(t-1)+\dots+a_{n_a}y(t-n_a)+
b_1u(t-1)+\dots+b_{n_b}u(t-n_b)
+\xi(t)
=
\theta^T\phi(t)+\eta(t)
\]
where $\theta=(a_1,\dots,a_{n_a},b_1,\dots,b_{n_b})$ and $\phi(t)=y(t-1),\dots,y(t-n_a),u(t-1),\dots,u(t-n_b)$ We then use the MSE as optimization criterion and impose its detivative to 0, obtaining the normal equations:
\[
\sum_{t=1}^N \phi(t)\phi^T(t)\theta
=
\sum_{t=1}^N y(t)\phi^T(t)
\implies
\hat{\theta}=
\left[
\sum_{t=1}^N \phi(t)\phi^T(t)
\right]^{-1}
\cdot
\sum_{t=1}^N y(t)\phi^T(t)
\]
\paragraph{Toepliz matrix} 
$
R_{uu}=
\begin{vmatrix}
...
\end{vmatrix}
$ 
If $R_{uu}$ is invertible, then the normal equations will have a unique solution.
\subsection{Estimation of mean, variance, spectrum}
Given the data $y_1,\dots,y_N$, we want to calculate an estimator $\hat{\bullet}_N$ (or $\hat{\bullet}$) from the data 
\subsubsection{Mean} 
\[
\hat{m}_n=\frac{1}{N}\sum_{i=1}^N y_i
\]
\begin{description}
\item[Correct] $E[\hat{m}_N]=m \quad \forall N$
\item[Consistent] $N \to \infty \implies Var[\hat{m}_s] \to 0$
\end{description}
\subsubsection{Covariance (a)}
\[
\hat{\gamma}_N^a(\tau)=
\frac{1}{N} \sum_{i=1}^{N-\abs{\tau}} y_i y_{i+\abs\tau}
\]
\begin{description}
\item[Has positive semi-definite Toeplitz matrix]
\item[Asymptotically correct] $N \to \infty \implies E[\hat{\gamma}_N^a(\tau)] \to \gamma(\tau)$
\end{description}
\subsubsection{Covariance (b)}
\[
\hat{\gamma}_N^a(\tau)=
\frac{1}{N-\abs\tau} \sum_{i=1}^{N-\abs{\tau}} y_i y_{i+\abs\tau}
\]
\begin{description}
\item[Correct] $E[\hat{\gamma}^b_N]=m \quad \forall N$
\end{description}
\subsubsection{Spectrum}
\[
\hat{\Gamma}(\omega_k)=
\sum_{\tau=1-N}^{N-1} \hat{\gamma}_N(\tau)e^{-j\omega_k \tau}
\]
\begin{description}
\item[Asymptotically correct] $N \to \infty \implies E[\hat{\Gamma}(\omega)] \to \Gamma(\omega)$
\item[Not correct] $N \to \infty \implies Var[\hat{\Gamma}(\omega)] \to \Gamma(\omega)^2$
\end{description}
\paragraph{Barlett method} Given $N$ data (with $N$ large) divide it into $r$ subsets of size $\hat{N}=\frac{N}{r}$ where $N>>r$. Compute the single spectra:
\[
\hat{\Gamma}_{\hat{N}}^{(i)} (\omega) \forall i=1,...,r
\]
and the average:
\[
\overline{\hat{\Gamma}}(\omega)=
\frac{1}{r} \sum_{i=1}^r \hat{\Gamma}_{\hat{N}}^{(i)}(\omega)
\]
As $N>>r$, data from different series can be assumed uncorrelated. We can then consider
\[
Var \left[\overline{\hat{\Gamma}}(\omega)\right]
\approx
\frac{1}{r} \Gamma^2(\omega) << \Gamma^2(\omega)
\]
...
\begin{itemize}
	...
	\item $\abs{\gamma(\tau)}\leq\gamma(0)		\quad\forall\tau$
\end{itemize}
\paragraph{White noise} $\eta(t)\sim WN(m,\lambda^2)$
\begin{itemize}
	\item Stationary process
...
\paragraph{Spectrum}
\begin{itemize}
\item $\Gamma(\omega)=\gamma(0)+2cos(\omega)\gamma(1)+2cos(2\omega)\gamma(2)+\dots$
...
\begin{itemize}
\item $\Gamma_{\text{out}}(\omega)=\abs{W(e^{j\omega})}^2 \cdot \Gamma_{\text{in}}(\omega)$
\item $\Phi_{\text{out}}(z)=W(z)W(z^{-1}) \cdot \Phi_{\text{in}}(z)$
\end{itemize}
\section{Moving Average MA(n)}
\begin{itemize}
\item $W(z)=\frac{c_0z^n+c_1z^{n-1}+\dots+c_n}{z_n}$
\item $m=0$
\item $\gamma(\tau)= 
\begin{cases}
(c_0c_\tau+c_1c_{1+\tau}+\dots+c_{n-\tau}c_\tau)\lambda^2	&	\abs{\tau}\leq n\\
0	& \text{otherwise}
\end{cases}$
\end{itemize}
\subsection{MA($\infty$)}
\begin{itemize}
\item $\gamma(0)=(c_0^2+c_1^2+\dots+c_k^2+\dots)\lambda^2$
\item $\gamma(0)$ must converge to a finite value
\end{itemize}
\section{Auto Regressive AR(n)}
\begin{itemize}
\item $m=0$
\item $W(z)=\frac{z^n}{z^n-a_1z_{n-1}-\dots-a_n}$
